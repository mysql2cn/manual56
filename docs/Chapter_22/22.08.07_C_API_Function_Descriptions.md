...
> 各参数的意义跟 *mysql_real_connect()* 相同，除了connection可以为 **NULL** 。再次情况下，C API会为该结构自动分配内存，同时在 *mysql_close()* 被调用的时候将其释放。这一方式的一个缺点是如果连接失败，你无法获得错误信息。（如果要通过 *mysql_errno()* 或者 *mysql_error()* 来获取错误信息，必须提供一个有效的 **MYSQL** 指针。）
...
> 但是如果你调用了 *mysql_use_result()* 初始化结果集读取时，各行是通过重复调用  *mysql_fetch_row()* 一行一行地从服务端读取回来的。因为这一过程中数据库连接上可能会发生错误，所以 *mysql_fetch_row()* 调用返回 **NULL** 并不意味着到达了结果集尾部。此时你可以使用 *mysql_eof()* 来确定到底发生了什么，如果返回非0说明确实到达了结果集尾部，返回0说明有错误发送。
...
> 一条规律是所有必须查询服务端以获取信息的函数在成功的时候都会重置 *mysql_errno()* 的返回值。
...
> 该函数功能跟 *mysql_real_escape_string()* 一样，除了后者会接收连接句柄作为第一个参数，并且依据其指定的当前字符集来进行转义。而 *mysql_escape_string()* 并不会接收这样的一个参数，也不会参考当前字符集。
...
> 每当你重新执行一句 **SELECT** 查询，该函数都会被重置为结果集返回第一列的信息。该函数的返回值还会受 *mysql_field_seek()* 调用的影响。

> 假设你调用了 *mysql_query()* 执行了一次 **SELECT** 查询但是还没有调用 *mysql_store_result()*，此时你如果调用 *mysql_fetch_field()* 来获取某一 **BLOB** 字段的长度，则返回值将会是该类型字段的默认长度（8KB）。（之所以是8KB是因为MySQL并不知道结果集中该类型自动的最大长度。这个数字将来会变成可配置的。）一旦你读取了结果集，*field->max_length* 就会包含该查询中这一列的最大长度。
...
> 以 *MYSQL_FIELD* 结构体的形式返回列编号 *fieldnr* 结果集内指定列的定义。可以使用该函数来获取任意列的信息。 *fieldnr* 应该介于0与 * mysql_num_fields(result)-1* 之间。
...
> 返回一个 **MYSQL_FIELD** 结构的一个列表。每个结构提供结果集中一列的定义。
...
> 返回结果集中当前行各列的长度。如果你要拷贝各列的内容，这些信息可以拿来优化代码，因为这样你可以避免调用 *strlen()* 。另外，如果结果集包含了二进制数据，你只能使用这一方法来确定数据的大小，因为对于任何包含了'\0'的字段，*strlen()* 都会返回错误的结果。
...
> 结果集中的一行有多少列可以调用 *mysql_num_fields(result)* 获得。如果 *row* 存放了 *mysql_fetch_row()* 调用的返回值，各列值的指针可以用 *row[0]* 到 *row[mysql_num_fields(result)-1]* 的方式获得。 **NULL** 指以 **NULL** 指针表示。

> 各列的长度可以调用 *mysql_fetch_lengths()* 获得。空字段或者值为 **NULL** 的长度均为0；可以通过检查列值的指针来区分这两者。如果指针为 **NULL* ，则字段值为 **NULL** ；否则，字段仅为空而已。

**返回值**

> 表示下一行的 *MYSQL_ROW* 结构。如果所有行都已读取完毕或者有错误发生时返回 **NULL** 。

**错误**

> 注意在 *mysql_fetch_row()* 调用之间错误信息不会被重置。
...
> 该函数的使用场景是当调用 **mysql_store_result()* 时返回 **NULL** （此时你没有结果集指针）。此时，你可以调用 *mysql_field_count()* 来决定 *mysql_store_result()* 是否应该返回非空结果集。从而使得客户端无需知道查询是否是 **SELECT** 类型的，也能采取合适的的动作。下面的例子会说明如何做。
...
> 移动列游标到指定偏移处。下一个 *mysql_fetch_field()* 调用就将返回该偏移对应的列定义信息。
...
> 该函数返回一个到服务端的指定连接的SSL加密方法。 *mysql* 代表连接句柄，由 *mysql_init()* 返回。

**返回值**

> 一个表示连接所使用的SSL加密方法的字符串，如果没有加密则返回 **NULL** 。
...
> 该函数用来创建合法SQL字符串，以便能用在SQL之中。参考 [9.1.1，字符串表示]() 。

> *from* 所代表的字符串会被转换成16进制格式，每个字符被编码成2个16进制数字。结果末尾添加一个'\0'字节，存于 *to* 。

> *from* 指向的字符串必须时 *length* 字节长。 *to* 分配的内存至少应该有 _2*length+1_ 字节。当调用返回时， *to* 变量的内容为以null结尾字符串。返回值时编码字符串的长度，不包括结尾'\0'在内。
...
> 要注意 *mysql_info()* 只为 **INSERT ... VALUES** 语句的多行形式返回非 **NULL** 值（也就是说语句中列出了多个行）。
...
> 分配或者初始化一个用于 *mysql_real_connect()* 的 **MYSQL** 对象。如果 *mysql* 参数为 **NULL** 指针，该函数会分配并初始化一个新对象返回。否则会初始化传入的对象并返回对象的地址。 *mysql_init()* 分配新对象，调用 *mysql_close()* 以关闭连接的时候该对象将会被释放。

**返回值**

> 一个初始化了的 _MYSQL*_句柄。当内存不够，无法分配新对象的时候返回 **NULL** 。
