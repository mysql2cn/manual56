...
MySQL本身知道服务端的SQL值的类型。 **MYSQL_BIND** 结构的 *buffer_type* 成员表示客户端存有值的C变量的类型码。如果需要转换，这两者一起告诉MySQL应该如何转换。以下是几个例子：
...
* 对于 **MYSQL_TYPE_BIT** 类型码， **BIT** 值将被写入到字符串缓冲区，从而相应的C类型就为 *char[]* 。位字符串需要在客户端做一些解析工作。为了容易处理返回值，可以使用以下两种方式将其值转换成整数类型：
...
在将变量绑定到 **MYSQL_BIND** 结构以读取字段值之前，可以先检查结果集各字段的类型码。这对于选择最合适的变量类型，避免类型转换是很有帮助的。要获取类型码，应该在调用 *mysql_stmt_execute()* 之后调用 *mysql_stmt_result_metadata()* 。这些元数据是的访问结果集的类型码成为可能，详细参考 [22.8.11.23，mysql_stmt_result_metadata()]() 以及 [22.8.5，C API 数据结构]()。
...
预处理语句相关的表或者视图的元数据更改能够被自动探测到，进而在下次执行时对语句进行重新处理。更多信息参考 [8.9.4，预处理语句以及存储程序的缓存]() 。