...
也可以使用 [CHECK TABLE][13.07.02.02] 和 [REPARE TABLE][13.07.02.05] 语句来检查和修复MyISAM表。 参见 [13.7.2.2, CHECK TABLE 语法][13.07.02.02] 和 [13.07.02.05, REPAIR TABLE 语法][13.07.02.05].
...
>然而，避免这个问题最简单的方法是使用 [CHECK TABLE][13.07.02.02] 来代替 [myisamchk](#) ,参见 [13.7.2.2 CHECK TABLE 语法][13.07.02.02]
 ...
 [myisamchk](#) 支持如下的这些选项，可以在命令行中指定它们或者是在配置文件中以 [myisamchk] 选项组的形式指定。[myisamchk](#) 也支持处理配置文件的选项，参见[4.2.3.4 影响配置文件处理的命令行选项][04.02.03.04]。
