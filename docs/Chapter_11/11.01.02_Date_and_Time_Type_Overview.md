...
时间列类型摘要如下。有关时间列类型的性能和存储要求的详细信息，参见[11.3节，“日期和时间类型”](./11.03.00_Date_and_Time_Types.md)，[11.6节，“列类型存储需求”](./11.06.00_Data_Type_Storage_Requirements.md)。时间值的函数使用说明，参见[12.7节，“日期和时间函数”](../Chapter_12/12.07.00_Date_and_Time_Functions.md)。
...
在使用YEAR(2)类型之前应该考虑它的一些问题。从MySQL5.6.6开始，YEAR(2）已经过时。YEAR(2)列在当前表中被视为YEAR(2)，但在创建新表或表变更后将会被转换为YEAR(4)。更多详细信息，参见[11.3.4节，“YEAR(2)的局限性和转换至YEAR(4)”](./11.03.04_YEAR2_Limitations_and_Migrating_to_YEAR4.md)。
...
* DATETIME[( *fsp* )]
    ...
    从MySQL5.6.5开始，自动初始化和更新到当前日期时间，可以使用DATETIME列的DEFAULT和ON UPDATE定义项，详细描述参见[11.3.5节，“TIMESTAMP和DATETIME类型的自动初始化与更新”](./11.03.05_Automatic_Initialization_and_Updating_for_TIMESTAMP_and_DATETIME.md)。
...
* TIMESTAMP[( *fsp* )]
    ...
    服务器处理时间戳定义的方式取决于explicit\_defaults\_for\_timestamp[496]系统变量的值参见[5.1.4节，“服务器系统变量”](../Chapter_05/05.01.04_Server_System_Variables.md)。默认情况下， explicit\_defaults\_for\_timestamp [496]为禁用状态，服务器处理TIMESTAMP如下：
    ...
    从MySQL5.6.6开始，explicit\_defaults\_for\_timestamp[496]被禁用。MySQL5.6.6之前，服务器商讨禁用explicit\_defaults\_for\_timestamp [496]来处理TIMESTAMP。这些动作，虽然仍是默认的，是非标准的，是以前的5.6.6。有关升级与explicit\_defaults\_for\_timestamp[496]的安装与启用，参见[2.11.1.1节，“从MySQL5.5升级到5.6”](../Chapter_02/02.11.01_Upgrading_MySQL.md#1)。
...
* YEAR[(2|4)]
    ...
    > **提示**
    > 
    > YEAR(2)在使用前应考虑它存在一些问题。从MySQL5.6.6开始，YEAR(2)就过时了。在现有表中的YEAR(2)视为YEAR(2)，但新建的表和结构表更的表会将其转换为YEAR(4)。详细信息，参见[11.3.4节，“YEAR(2)的局限性和转换至YEAR(4)”](./11.03.04_YEAR2_Limitations_and_Migrating_to_YEAR4.md)。

    有关年的显示格式与赋值说明的详细信息，参见[11.3.3节，“YEAR类型”](./11.03.03_The_YEAR_Type.md)。
