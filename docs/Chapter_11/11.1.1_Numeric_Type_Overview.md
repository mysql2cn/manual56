...
下面为数值列类型的概述。有关数值类型的属性和存储性能的要求，详细信息参见[11.2节“数值类型”](./11.2.0_Numeric_Types.md)，列存储需求参见[11.6节“列类型存储需求”](./11.6.0_Data_Type_Storage_Requirements.md)。
...
> **警告**
> 
> 当使用在整数值（其中一个是UNSIGNED类型）之间使用减号时，结果是无符号的，除非NO_UNSIGNED_SUBTRACTION[621]SQL模式被启动。详情参见[12.10章节“Cast函数和操作符”](../Chapter_12/12.10.0_Cast_Functions_and_Operators.md)。
